...
import { ServerError, handleServerError } from "@/lib/server-error";
import { withUserRateLimit, withPublicRateLimit } from "@/lib/rate-limit-middleware";

/** Returns the public Razorpay key_id so the browser can open Checkout. */
export const getRazorpayKey = createServerFn({ method: "GET" })
  .middleware([withPublicRateLimit])
  .handler(async () => {
...
export const createRazorpayOrder = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth, withUserRateLimit])
  ...
  .handler(async ({ data, context }) => {
    return handleServerError(async () => {
      ...
      if (!keyId || !keySecret) {
        throw new ServerError("Payment processing is temporarily unavailable.", {
          internal: "RAZORPAY_KEY_ID or RAZORPAY_KEY_SECRET is not set",
        });
      }

      // Log key prefix so we can confirm live vs test in server logs
      ...
      if (!res.ok) {
        ...
        console.error(`[Razorpay] Order creation failed (key: ${keyId.slice(0, 14)}...):`, text);
        throw new ServerError("Payment processing failed. Please try again.");
      }
...
export const verifyRazorpayPayment = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    return handleServerError(async () => {
      ...
      if (!secret) {
        throw new ServerError("Payment verification is temporarily unavailable.", {
          internal: "RAZORPAY_KEY_SECRET is not set",
        });
      }
...
export const getResourceDownloadUrl = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    return handleServerError(async () => {
      ...
      // Validate the service-role key early
      if (!process.env.SUPABASE_SERVICE_ROLE_KEY) {
        throw new ServerError("Service temporarily unavailable.", {
          internal: "SUPABASE_SERVICE_ROLE_KEY is not set",
        });
      }
